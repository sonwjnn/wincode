...
describe("file mention resolver", () => {
	test("resolves file mentions to bounded data parts", async () => {
		...
		await mkdir(path.join(workspace, "packages/tui"), { recursive: true });
		await writeFile(
			path.join(workspace, "packages/tui/file.ts"),
			"export const x = 1;"
		);

		const parts = await resolveFileMentionParts("fix @packages/tui/file.ts", {
			root: workspace,
		});
	...
	test("resolves directory mentions as bounded trees", async () => {
		const workspace = await createWorkspace();
		await mkdir(path.join(workspace, "packages/tui/src"), { recursive: true });
		await writeFile(path.join(workspace, "packages/tui/package.json"), "{}");
		await writeFile(path.join(workspace, "packages/tui/src/index.ts"), "");

		const [part] = await resolveFileMentionParts("inspect @packages/tui/", {
			maxDirectoryDepth: 1,
			root: workspace,
		});
		...
		expect(part?.data.content).toContain("packages/tui/");
	...
	test("resolves a unique basename fallback to its canonical path", async () => {
		...
		await mkdir(path.join(workspace, "packages/tui/src"), { recursive: true });
		await writeFile(
			path.join(workspace, "packages/tui/src/bot-message.tsx"),
	...
	test("reports bounded ambiguity instead of choosing a basename match", async () => {
		...
		await writeFile(path.join(workspace, "packages/tui/index.ts"), "apps");
		...
		expect(part?.data.error).toContain("packages/tui/index.ts");
