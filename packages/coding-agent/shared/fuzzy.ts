...
function buildSearchIndex(text: string): SearchIndex {
	...
	if (text.length > MAX_CACHED_TEXT_LEN) {
		return buildUncachedSearchIndex(text);
	}
...
function buildAlphanumericSwapQueries(queryLower: string): string[] | null {
	...
	for (let i = 0; i < queryLower.length - 1; i++) {
		...
		const isAlphaNumSwap =
			(current &&
				LOWERCASE_LETTER_RE.test(current) &&
				next &&
				DIGIT_RE.test(next)) ||
			(current &&
				DIGIT_RE.test(current) &&
				next &&
				LOWERCASE_LETTER_RE.test(next));
		if (!isAlphaNumSwap) {
			continue;
		}
		const swapped =
...
function scoreTokenAgainstWord(
	token: string,
	word: SearchWord
): FuzzyMatch | null {
	if (word.text === token) {
	...
	if (word.text.startsWith(token)) {
		return {
			matches: true,
			score: withPosition(
				-170 + (word.text.length - token.length) * 0.5,
				word.index
			),
		};
	}
	...
	if (token.startsWith(word.text) && token.length - word.text.length <= 2) {
		return {
			matches: true,
			score: withPosition(-150 + token.length - word.text.length, word.index),
		};
	}
	...
	if (substringIndex >= 0) {
		return {
			matches: true,
			score: withPosition(-20 + substringIndex, word.index),
		};
	}
	...
	if (characterMatch.span > maxSpan) {
		return null;
	}

	return {
		matches: true,
		score: withPosition(-40 + characterMatch.score, word.index),
	};
}

function scoreAcronym(token: string, index: SearchIndex): FuzzyMatch | null {
	if (token.length < 2 || token.length > 4 || index.words.length === 0) {
		return null;
	}
	...
	if (queryIndex < token.length || firstOrdinal < 0 || lastOrdinal < 0) {
		return null;
	}

	const wordSpan = lastOrdinal - firstOrdinal + 1;
	if (wordSpan > token.length + 2) {
		return null;
	}

	return {
		matches: true,
		score: withPosition(-30 + wordSpan * 4 - token.length * 2, firstTextIndex),
	};
}
