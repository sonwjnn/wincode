...
const environment = {
	stable: {
		...
		worktree: "packages/coding-agent",
		workspace: "/repo",
	},
	volatile: {
		branch: "main",
...
describe("Prompt Composition", () => {
	...
	test("loads ancestor AGENTS.md files root-first and isolates invalid sources", async () => {
		const files = {
			...
			"/repo/packages/coding-agent/AGENTS.md": new Uint8Array([0xc3, 0x28]),
		};
		const reads: string[] = [];
		const snapshot = await createProjectInstructionSnapshot({
			fs: fileSystem(files, metadataFor(files), reads),
			projectRoots,
			workspace: "/repo/packages/coding-agent",
		});
	...
	test("discovers nested instructions through the active working directory", async () => {
		const files = {
			"/repo/AGENTS.md": "Repository defaults",
			"/repo/packages/AGENTS.md": "Package defaults",
			"/repo/packages/coding-agent/AGENTS.md": "Workspace defaults",
		};
		const service = createPromptCompositionPipeline();
	...
	test("omits oversized files without truncating valid sources", async () => {
		const files = {
			...
			"/repo/packages/coding-agent/AGENTS.md": "t".repeat(1000),
			"/repo/packages/coding-agent/src/AGENTS.md": "s".repeat(12_001),
		};
		const snapshot = await createProjectInstructionSnapshot({
			fs: fileSystem(files),
			projectRoots: [
	...
	test("omits unreadable instruction sources while retaining readable neighbors", async () => {
		const files = {
			...
			"/repo/packages/coding-agent/AGENTS.md": "workspace",
		};
		const fs = {
