...
describe("BotMessageContent shell output", () => {
	...
	test("renders the command summary and output block collapsed by default", async () => {
		...
		expect(frame).not.toContain("2 passing");
	});
	...
	test("renders timeout and truncation markers", async () => {
		...
		expect(frame).toContain("▸ Output · timed out (click to expand)");
		expect(frame).toContain("▸ Output · exit 1 · truncated (click to expand)");
	});
	...
	test("expands the collapsed output block on click and re-collapses on a second click", async () => {
		const setup = await testRender(
			<ThemeProvider>
				<KeyboardLayerProvider>
					<ApprovalPanelsProvider>
