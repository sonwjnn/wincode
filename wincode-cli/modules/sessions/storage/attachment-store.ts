...
type AttachmentMetadataRow = typeof sessionAttachment.$inferSelect;
...
export const createDrizzleAttachmentMetadataRepository = (
	...
	delete: (attachmentId) => {
		db.delete(sessionAttachment)
			.where(eq(sessionAttachment.attachmentId, attachmentId))
			.run();
	},
	get: (attachmentId) => {
		const row = db
	...
	list: () =>
		db.select().from(sessionAttachment).all().map(toAttachmentMetadataRecord),
	put: (record) => {
		db.insert(sessionAttachment)
			.values({
...
export type SessionAttachmentStoreOptions = {
	maxBytes?: number;
	now?: () => Date;
...
export const stripAttachmentDisplayMetadata = (
	...
	return durablePart as SessionMessage["parts"][number];
};
...
export const messageHasLegacyImageParts = (message: SessionMessage): boolean =>
	message.parts.some(isLegacyImagePart);
...
const hydrateMessageParts = (
	...
	purpose: AttachmentHydrationPurpose
): SessionMessage["parts"] | undefined => {
	let changed = false;
...
export const createSessionAttachmentStore = ({
	maxBytes = MAX_ATTACHMENT_BYTES,
	now = () => new Date(),
	repository,
	root,
}: SessionAttachmentStoreOptions): SessionAttachmentStore => {
	const ingest = async (
		input: AttachmentInput,
		signal?: AbortSignal
	): Promise<AttachmentReference> => {
