...
describe("SessionOperation", () => {
	...
	test("cancels the active send through its abort signal", async () => {
		...
		const execute = mock(
			(_input: SessionSendInput, signal: AbortSignal) =>
	...
	test("maps a deadline timer to a deadline abort reason", async () => {
		const execute = mock(
			async (_input: SessionSendInput, signal: AbortSignal) => {
	...
	test("marks an explicit interrupt distinctly from cancellation", async () => {
		...
		const execute = mock((_input: SessionSendInput, signal: AbortSignal) => {
			started.resolve();
			const { promise, resolve } = Promise.withResolvers<{
				rejected: true;
				reason: string;
		...
		const operation = createSessionOperation({ execute, onInterrupt });
		const send = operation.send(request);
	...
	test("preserves a gated Tool Call completion through the application seam", async () => {
		...
		const committedRecords: Array<{
			...
			attachments: NonNullable<SessionSendInput["files"]>;
			model: SessionSendInput["model"];
			toolResult: string;
		}> = [];
		const projectedOutcomes: string[] = [];
		const execute = mock(
			async (input: SessionSendInput, signal: AbortSignal) => {
				if (signal.aborted) {
