...
describe("file mention resolver", () => {
	test("resolves file mentions to bounded data parts", async () => {
		...
		await mkdir(path.join(workspace, "wincode-cli"), { recursive: true });
		await writeFile(
			path.join(workspace, "wincode-cli/file.ts"),
			"export const x = 1;"
		);

		const parts = await resolveFileMentionParts("fix @wincode-cli/file.ts", {
			root: workspace,
		});
	...
	test("resolves directory mentions as bounded trees", async () => {
		const workspace = await createWorkspace();
		await mkdir(path.join(workspace, "wincode-cli/src"), { recursive: true });
		await writeFile(path.join(workspace, "wincode-cli/package.json"), "{}");
		await writeFile(path.join(workspace, "wincode-cli/src/index.ts"), "");

		const [part] = await resolveFileMentionParts("inspect @wincode-cli/", {
			maxDirectoryDepth: 1,
			root: workspace,
		});
		...
		expect(part?.data.content).toContain("wincode-cli/");
	...
	test("resolves a unique basename fallback to its canonical path", async () => {
		...
		await mkdir(path.join(workspace, "wincode-cli/src"), { recursive: true });
		await writeFile(
			path.join(workspace, "wincode-cli/src/bot-message.tsx"),
		...
		expect(part?.data).toMatchObject({
			...
			kind: "file",
			path: "wincode-cli/src/bot-message.tsx",
		});
	});
	...
	test("reports bounded ambiguity instead of choosing a basename match", async () => {
		...
		await writeFile(path.join(workspace, "wincode-cli/index.ts"), "apps");
		...
		expect(part?.data.error).toContain("wincode-cli/index.ts");
