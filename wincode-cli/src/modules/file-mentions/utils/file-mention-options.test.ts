...
describe("file mention options", () => {
		...
		expect(
			filterFileMentionOptions(
				[
					...
					{
						label: "wincode-cli/src/index.ts",
						path: "wincode-cli/src/index.ts",
						type: "file",
					},
				],
				"cli"
	...
	test("ranks basename matches ahead of parent-directory matches", () => {
		const options = [
			...
			{
				label: "wincode-cli/src/bot-message.tsx",
		...
		expect(
			...
			"wincode-cli/src/bot-message.tsx",
	...
	test("matches extensionless stems and subsequence abbreviations", () => {
		...
		const options = [
			botMessageOption,
			{
				label: "wincode-cli/src/other.ts",
				path: "wincode-cli/src/other.ts",
				type: "file" as const,
			},
		];
	...
	test("keeps slash-containing queries in path context", () => {
		const options = [
			...
			{
				label: "wincode-cli/src/",
				path: "wincode-cli/src",
				type: "directory" as const,
			},
			{
				label: "wincode-cli/src/index.ts",
		...
		expect(
			filterFileMentionOptions(options, "wincode-cli/").map(
				(option) => option.path
			)
		).toEqual(["wincode-cli/src", "wincode-cli/src/index.ts"]);
