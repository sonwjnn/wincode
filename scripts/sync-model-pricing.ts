...
`;
	...
	const variantsByModel: Record<string, readonly ModelVariant[]> = {};
	for (const model of supportedChatModels) {
		if (model.connectionProviderId !== "opencode-go") {
			continue;
		}
		const entry = goProvider?.models?.[model.id];
		variantsByModel[model.id] = toModelVariants(
	...
	const variantsPath = join(
		dirname(new URL(import.meta.url).pathname),
		"..",
		"packages/ai/src/generated/opencode-go-variants.generated.ts"
	);
	mkdirSync(dirname(variantsPath), { recursive: true });
	const variantsBody = `// Generated by scripts/sync-model-pricing.ts — do not edit.
// Snapshot date: ${date}. Source: ${url}.

export const openCodeGoVariantsByModel = ${JSON.stringify(variantsByModel, null, 2)} as const;
`;
