...
- **Runtime diagnostics stay out of CLI and protocol streams.** Runtime code uses
  ...
  fields and URL query parameters are redacted, and logging failures are ignored
  rather than corrupting output or interrupting execution. RPC fatal diagnostics
  are also sent to stderr; JSON-RPC stdout remains protocol-only.
