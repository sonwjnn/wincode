...
| Wincode target | [`packages/coding-agent/shared/ui/progress-bar.tsx`](../../packages/coding-agent/shared/ui/progress-bar.tsx) |
...
Wincode's revised component uses a fixed width of twelve and an `80 ms` interval, moving a six-cell square trail across the bar and back with endpoint fade holds ([source](../../packages/coding-agent/shared/ui/progress-bar.tsx)). Agent-colored `■` cells use an opaque same-hue brightness trail—head at `1.0`, bloom at `1.15`, then `0.85`, `0.65`, `0.48`, and `0.32`—over a muted `⬝` track. Using opaque shades instead of increasingly transparent alpha avoids compositing the tail toward gray terminal backgrounds while preserving the agent hue.

The chat footer renders the progress component only while busy and follows it with an agent-colored `Esc` plus a muted interrupt hint ([source](../../packages/coding-agent/modules/sessions/ui/components/chat-shell.tsx#L156-L166)). This textual hint preserves meaning independently of color or motion.
