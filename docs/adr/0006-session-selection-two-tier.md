# Session Selection Records Two Tiers

The session-selection read path merges two intentionally distinct sources:
the session row records the **session-level choice** — the user's explicit
...
owned by the session-selection module and exercised by chat-view,
...
- **Two-tier write (accepted)** - `submit` and `continueLastMessage` carry the
  effective selection (`modelRef`/`variantRef`) and the session-level
  choice (`sessionModelRef`/`sessionVariantRef`) as separate values.
  ...
  `high` for one turn keeps `low` as the session choice: a new chat from
...
- **Single written value** - persist the effective selection to the session row
  and delete `sessionModelRef`/`sessionVariantRef` and the
  `sessionModel`/`sessionVariant` parameters. Removes the divergence
  ...
  not a session preference, so the collapse regresses restore semantics.
...
- Future restore paths must route through the session-selection merge
