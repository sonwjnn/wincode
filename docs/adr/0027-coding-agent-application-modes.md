...
- An Execution Mode is a Strategy supplied by the `bin/` composition root. Modes receive a shared application context and use the same Session Host and Session Engine contracts; they do not implement independent agent loops or session state owners. `modules/application/` never imports the concrete `tui/` adapter.
- `packages/coding-agent/index.ts` is the canonical application interface for package consumers and re-exports `modules/application/application.ts`. The package `.` export points both `types` and `import` at `index.ts`; there is no duplicate `./application` alias. `bin/`, `tui/`, and `modules/application/` remain implementation directories, while the explicit Session Host, capability, and RPC subpath exports expose only their UI-neutral contracts.
...
- The `bin/` composition root may load resources lazily. Help/version do not load the interactive runtime; Interactive Mode composes the renderer, while non-interactive modes use the React-free Session Host capability graph.
