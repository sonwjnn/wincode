...
- `@wincode/coding-agent` replaces the private `@wincode/cli` and `@wincode/tui` packages. The package owns CLI parsing, help/version, process exit-status mapping, application composition, OpenTUI, persistence, connections, MCP, permissions, Session Hosts, and mode adapters.
- The short flags are `-m` for `--mode` and `-p` for `--prompt`; mode values remain full names, with no `p`/`j`/`r` mode aliases.
- An Execution Mode is a Strategy selected by a factory/registry. Modes receive a shared application context and use the same Session Host and Session Engine contracts; they do not implement independent agent loops or session state owners.
- Interactive Mode is the default bare `wincode` invocation. Print and JSON are one-shot modes that open or create a durable One-Shot Session, accept exactly one Submission from `--prompt`/`-p` or stdin, and end after the terminal Agent Turn outcome. JSON emits the public Agent Turn event projection as JSONL; Print emits only human-readable assistant text.
...
- The application package may compose resources lazily. Help/version do not load the interactive runtime; Interactive Mode composes the renderer, while non-interactive modes use the React-free Session Host capability graph.
